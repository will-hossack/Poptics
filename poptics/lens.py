...
"""
from poptics.surface import OpticalPlane,FlatSurface,ImagePlane,CircularAperture,IrisAperture,\
    AnnularAperture,SphericalSurface,QuadricSurface,ParabolicSurface,SphericalImagePlane
...
class OpticalGroup(list):
    ...
    def add(self,surface):
        ...
        if isinstance(surface,OpticalGroup):        # Unpack OpticalGroup
            ...
            if isinstance(surface,CircularAperture): # Record location of aperture
                self.aperture = surface

            if isinstance(surface,IrisAperture):     # Record location of iris
                self.iris = surface
    ...
    def planePair(self,mag, xsize = 100.0, ysize = None , wave = None):
        ...
        obj = ImagePlane([pt.x,pt.y,pobj.inputPlane()],xsize,ysize)
        ima = ImagePlane([pt.x,pt.y,pima.inputPlane()],xsize*abs(mag),ysize*abs(mag))
        return obj,ima
    ...
    def entranceAperture(self):
        ...
        if isinstance(s,QuadricSurface):  # If curved surface, find the edge plane and add/subsract it
            pt.z += s.edgePlane()

        return CircularAperture(pt,s.maxRadius)
    ...
    def exitAperture(self):
        ...
        if isinstance(s,QuadricSurface):   # If curved surafce, find surface edge plane and add/subtract
            pt.z +=  s.edgePlane()
    ...
    def paraxialMatrix(self,wave = None , first = 0, last = -1):
        ...
        for s in self[first:last]:
            if isinstance(s,QuadricSurface) and s.type != 0:  # Curved surface and not clear
                zp = s.point.z
...
class Lens(OpticalGroup):
    ...
    def entrancePupil(self,wave = None):
        ...
        return CircularAperture(p,mag*self.aperture.maxRadius)
    ...
    def exitPupil(self,wave = None):
        ...
        return CircularAperture(p,mag*self.aperture.getRadius())
    ...
    def frontFocalPlane(self, wave = None):
        ...
        return OpticalPlane(Vector3d(self.point.x,self.point.y,zplane))

    def backFocalPlane(self, wave = None):
        """
        Get the back Focal Plane as an OpticalPlane is global coordinates.

        :param wave: the wavelength (Default = wl.Design)
        :type wave: float

        return OpticalPlane in global coordinates.
        """
        pm = self.paraxialGroup(wave)
    ...
    def frontPrincipalPlane(self,wave = None):
        """
        Get the front principal place as an OpticalPlane in gobal cordilanes
    ...
    def backPrincipalPlane(self,wave = None):
        """
        Get the back principal place in gobal cordilanes as an OpticalPlane in global coordinates
...
class Singlet(Lens):
    ...
    def __init__(self, pt_or_z = 0.0, cl = 0.01 , t= 5.0 ,cr = -0.01 ,rad = 10.0,index= "BK7"):
        ...
        sl = SphericalSurface(0.0,cl,rad,index)        # Front surface at 0.0 
        self.add(sl)
        sr = SphericalSurface(t,cr,rad,AirIndex())  # back surface at t
        self.add(sr)
...
class Eye(Lens):
    ...
    def __init__(self,pt = 0.0, pixels = 0):
        ...
        self.add(SphericalSurface(self.lensbackposition, self.lensbackcurvature, 3.0, \
        ...
        for line in lensfile.readlines():
            ...
            if not line.startswith("#") and len(line) > 0:   # Kill comments and blanks
                ...
                if token[0].startswith("title"):              # Deal with title
                    ...
                    s = AnnularAperture(p,inner,outer) 
                    self.add(s)
                    ...
                    if token[6].startswith("index"):          # refrective index
                        ...
                        s = SphericalSurface(p,c,r,index) 
                    elif token[6].startswith("mirror"):       # else its a mirror
                        s = SphericalSurface(p,c,r)
                    else:
                    ...
                    if token[6].startswith("index"):
                        index = MaterialIndex(token[7])
                        s = ParabolicSurface(p,c,r,index)
                    elif token[6].startswith("mirror"):
                        s = ParabolicSurface(p,c,r)
                    else:
                        raise IOError("unknow type")
                    self.add(s)
                    ...
                    if token[8].startswith("index"):
                        ...
                        s = QuadricSurface(p,c,r,e,index)
                    elif token[8].startswith("mirror"):
                        s = QuadricSurface(p,c,e,r)
                    else:
