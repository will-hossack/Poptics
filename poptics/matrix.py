"""
   ...
   These classes are standalone can be used without using the complexities of
   the full optics package.
"""
...
class ParaxialMatrix(object):
    """
    Class to implement the basic ParaxialMatrix with 4 float components and a
    thickness.
    ...
    def __str__(self):
        """
        Return string representation with the four components and thickess
        displayed in 7.5f format.
        """
    ...
    def inverse(self):
        """
        Return the inverse of the matrix as a new ParaxialMatrix, also the
        thickness will be negated.
    ...
    def scale(self, a):
        ...
        This is equivalent to scaling the optical component that the matrix
        represents.
        """
        self.B *= a
        self.C /= a
    ...
    def backFocalLength(self):
        """
        Get the back focal length, so focal length in image space (assumes the
        matrix is for imaging system)
