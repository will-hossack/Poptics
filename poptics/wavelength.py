...
def getInitialDefaultWavelength():
    """
    Function to extarct the default wavelength from environmental variable DEFAULTWAVELENGTH.
    It can be set to numerical value or any the wavelengths specified as Globals above.
    If environemntal variale not set, then thde default of Green = 0.55 is used

    This is called automatically once on startup so should not be called by users.
...
class WaveLength(object):
    ...
    - self.dynamic = False (forces new calculation for every call)
