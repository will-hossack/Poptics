...
The set of classes in poptics.matrix implement paraxial maxtrix analysis of optical systems. These classes are used
to perform all the paraxial calcualtions inside poptics, but are also usable as stand along module
to perform paraxial calculations without the need to access any of the more complex poptics fetures.
...
.. autoclass:: poptics.matrix.ParaxialMatrix
   :members:
...
.. autoclass:: poptics.matrix.PropagationMatrix
   :members:

.. autoclass:: poptics.matrix.DielectricMatrix
   :members:

.. autoclass:: poptics.matrix.ThinLensMatrix
   :members:

.. autoclass:: poptics.matrix.ThickLensMatrix
   :members:

.. autoclass:: poptics.matrix.DoubletMatrix
   :members:

.. autoclass:: poptics.matrix.MirrorMatrix
   :members:

.. autoclass:: poptics.matrix.CavityMatrix
   :members:
...
Note that with the basic ParaxialMatrix classes focal and principle plane location
are measure from the input and output planes. 

ParaxialGroup Class
===================

The ParaxialGroup class in an etension of ParaxialMatrix that adds input plane, output plane and maxium height
at input and output. The input and output planes are defined in global coordinates, so in mm along the optical axis.
The methods to obtain the imaging planes are defedined so that they also return
their position in global coordinates.

.. autoclass:: poptics.matrix.ParaxialGroup
   :members:
...
.. autoclass:: poptics.matrix.ParaxialThickLens
   :members:


.. autoclass:: poptics.matrix.ParaxialDoublet
   :members:


.. autoclass:: poptics.matrix.ParaxialMirror
   :members:

.. autoclass:: poptics.matrix.DataBaseMatrix
   :members:
...
The matrix algebra with the ParaxialGroups is a little more complex than above,
...
This will create ParaxialGroup for two thin lenses serarated by 10mm. Note the second
argument cannot be a ParaxialGroup.
...
The First line creates a thin lens of focal length 100mm, radius 10 located at 40mm along the optical axis.
The second a thin lens of focal length -80mm, radius 10, located at 60mm along the optical axis.
...
Note: the ParaxialGroups are assumed to be located along the optical axis so they do not
overlap with the distance between then being positive. If this is not true then the calculations all work,
but it will be the analysis of a physically impossible optical system.
...
This will create the PaxaialGroup of a thin lens of focal length 100mm located at 40mm along
the optical axis, and then plot the the location of the input, output, focal and principle planes
with an optional ledgend in the lower left of the plot.
